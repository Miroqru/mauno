...
def news(update: Update, context: CallbackContext):
    ...
    send_async(
        context.bot,
        update.message.chat_id,
        text="All news here: https://telegram.me/unobotnews",
        disable_web_page_preview=True,
    )
...
def stats(update: Update, context: CallbackContext):
    ...
    if not us or not us.stats:
        ...
        pr_win = (
            round((us.first_places / us.games_played) * 100)
            if us.games_played else 0
        )
        send_async(context.bot, update.message.chat_id, text=(
            f"{us.games_played} game played"
            f"{us.first_places} first place ({pr_win}%)"
            f"{us.cards_played} card played"
        ))
